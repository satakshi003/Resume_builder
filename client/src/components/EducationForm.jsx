...
const EducationForm = ({data, onChange}) => {
  ...
  <div className='space-y-6'>
      <div className="flex items-center justify-between">
        ...
        <button onClick={addEducation} className="flex items-center gap-2 px-2 py-1 text-sm bg-green-100 text-green-700 rounded-lg hover:bg-green-200 transition-colors ">
      ...
      <div className='space-y-4'>
        {data.map((education, index)=>(
          ...
          <div className='grid md:grid-cols-2 gap-3'>
            ...
            type='text' placeholder='Instituition  Name' className='px-3 py-2 text-sm ' />

             <input value={education.degree || ""} onChange={(e) => updateEducation(index, "degree", e.target.value)}
            type='text' placeholder="Degree (e.g., Bachelor's, Master's)" className='px-3 py-2 text-sm ' />
